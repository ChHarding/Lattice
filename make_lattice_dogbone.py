...
def make_stl(cyl_rad, gap, angle):
    ...
    for c in range(0, num_cyls):
        ...
        #c = scale((1, 3.2, 1))(c) # make y dimension always be 3.2
        cyl_lst.append(c)
    ...
    for i,c in enumerate(cyl_lst):
        ...
        # move right    
        tc = translate((offset + gap*i - hwidth, 0, 0))(rc)
    ...
    # make text and remove  (=> relief)
    txt = union()
    t = text(text=f"r{cyl_rad}", size=5, spacing=1.1)  # g{gap} a{angle}
    t = linear_extrude(height=1)(t)
    t = rotate((90,0,0))(t)
    t = translate((-6,-1, 75))(t)
    txt.add(t)
    t = text(text=f"g{gap}", size=5, spacing=1.1)  # g{gap} a{angle}
    t = linear_extrude(height=1)(t)
    t = rotate((90,0,0))(t)
    t = translate((-6, -1, 68))(t)
    txt.add(t)
    t = text(text=f"a{angle}", size=5, spacing=1.1)  # g{gap} a{angle}
    t = linear_extrude(height=1)(t)
    t = rotate((90,0,0))(t)
    t = translate((-6, -1, 61))(t)
    txt.add(t)
    pdiff.add(txt)
    ...
    base_fname = f"lattice_dogbone_r{cyl_rad}_g{gap}_a{angle}"
...
# these setting will end up in the filename
rad = 1.6
gap = 15   
angle = 45
